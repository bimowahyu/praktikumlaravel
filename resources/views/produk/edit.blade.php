...
<div class="container">
    ...
    <form method="post" action="{{ url('/produk/update/'.$produk->id) }}" enctype="multipart/form-data">
        ...
        <div class="row mb-3">
            ...
            <div class="col-9">
                <input type="text" class="form-control" id="rasa" name="rasa" value="{{ old('rasa', $produk->rasa) }}"/>
            </div>
        </div>
        <div class="row mb-3">
            <label for="ukuran" class="col-3 col-form-label">Ukuran (ml)</label>
            <div class="col-9">
                <input type="text" class="form-control" id="ukuran" name="ukuran" value="{{ old('ukuran', $produk->ukuran) }}"/>
            </div>
        </div>
        <div class="row mb-3">
            <label for="berat" class="col-3 col-form-label">Berat (gr)</label>
            <div class="col-9">
                <input type="text" class="form-control" id="berat" name="berat" value="{{ old('berat', $produk->berat) }}"/>
            </div>
        </div>
        <div class="row mb-3">
        ...
        <div class="row mb-3">
            <div class="col-5 offset-md-3">
                <img style="width:100%" src="{{ url('/storage', $produk->image_url) }}" />
            </div>
        </div>
        <div class="row mb-3">
            <div class="col-5 offset-md-3 gap-2 d-grid">
                <button type="button" class="btn btn-danger btn-block" onclick="hapus_gambar()">HAPUS GAMBAR</button>
            </div>
        </div>
        @endif
        <div class="row">
            <div class="col-md-12">
                ...
                <a href="{{ url('/produk') }}" class="btn btn-large btn-secondary">Batal</a>
            </div>
        </div>
    </form>
    <form action="{{ url('/produk/destroy_image/'.$produk->id) }}" method="post" id="destroy_image">
        @csrf
    </form>
</div>
<script>
